...
# First binary classification problem to discriminate Alzheimer Disease vs. Control patients
...
val_pred_out <- predict(model_outs, valid_samples[, -430])
...
val_pred_no_out <- predict(model_no_outs, valid_samples[, -430])
...
write.csv(test_preds, file = "0075715_Amin_ADCTLres.csv", row.names = FALSE)
write.csv(features, file = "0075715_Amin_ADCTLfeat.csv", row.names = FALSE)

# Save predictions 
save(test_preds,  file = "0075715_Amin_ADCTLres.RData")
# Save feature indices 
save(features,  file = "0075715_Amin_ADCTLfeat.RData")
